...
namespace
{
    // No Mid pot on the real amp - see header - a fixed 6,800-ohm
    // resistor position ("equivalent to a mid pot set at 68%") stands in
    // for it.
    constexpr float fixedMidAmount = 0.68f;
...
FenderDeluxeReverbPreampPedal::FenderDeluxeReverbPreampPedal() = default;

void FenderDeluxeReverbPreampPedal::prepare(double sampleRate, int /*maximumBlockSize*/, int numChannels)
{
    ...
    for (int ch = 0; ch < juce::jmax(1, numChannels); ++ch)
    {
        ...
        toneStacks.emplace_back(sampleRate); // default Components, same blackface values as the Twin/Princeton
    }
}

void FenderDeluxeReverbPreampPedal::process(float* const* channelData, int numChannels, int numSamples)
{
...
std::vector<PedalParameter*> FenderDeluxeReverbPreampPedal::getParameters()
{
